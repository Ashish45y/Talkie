...
class NowPlayingViewModel @Inject constructor(
    ...
    private val _nowPlayingUiState = MutableStateFlow<UiState<List<MoviesDto>>>(UiState.Loading)
    val nowPlayingUiState : StateFlow<UiState<List<MoviesDto>>> = _nowPlayingUiState
    private fun checkInternetConnection(): Boolean = networkHelper.isNetworkConnected()
    init {
        if(checkInternetConnection()){
            fetchNowPlaying()
        }else{
         _nowPlayingUiState.value = UiState.Error("No Internet Connection")
        }
    }

    private fun fetchNowPlaying() {
        viewModelScope.launch(dispatcherProvider.main){ 
            nowPlayingRepo.getNowPlayingList("now_playing",1)
                .flowOn(dispatcherProvider.io)
                .catch { e ->
                    _nowPlayingUiState.value = UiState.Error(e.toString())
                }.map { 
                    it.map { moviesDto -> moviesDto}
                }.collect{
                    _nowPlayingUiState.value = UiState.Success(it)
                    logger.logger("nowPlaying","success")
                }
        }
    }
}